...
Escher.prototype = {
  ...
  signRequest: function(requestOptions, body, headersToSign) {
    if (requestOptions.method === undefined || requestOptions.method === null) {
    ...
    var defaultHeaders = utils.normalizeHeaders([
      ['host', requestOptions.hostname || 'localhost'],
  ...
  authenticate: function(request, keyDB, mandatorySignedHeaders) {
    ...
    if (isPresignedUrl) {
      ...
      request.url = uri.path + (canonicalizedQueryString ? '?' + canonicalizedQueryString : '');
    } else {
    ...
    if (!request.hostname) {
      request.hostname = utils.getHeader(request, 'host');
    }
  ...
  validateRequest: function(request, body) {
    ...
    if (request.url && (request.url.indexOf("http://") == 0 || request.url.indexOf("https://") == 0)) {
